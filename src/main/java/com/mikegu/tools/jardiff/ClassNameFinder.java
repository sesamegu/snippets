...
public class ClassNameFinder {
    ...
    public static void findName(String name){
        ...
        for (String kee :keySet){
            if (kee.contains(name)){
                List<ClassInfosDO> list = firstMap.get(kee);
                StringBuffer input2SB = new StringBuffer();
                for (ClassInfosDO oneClassInfosDO : list) {
                    input2SB.append(oneClassInfosDO.getClassFullName()+" " + oneClassInfosDO.getJarName() + ",");
                }
                System.out.println(input2SB.toString());
            }
        }
    }
    ...
    //        List<String> allClassName = new ArrayList<String>();
    //        getAllClassName("/tmp/1", allClassName, "/tmp/1".length() + 1);
